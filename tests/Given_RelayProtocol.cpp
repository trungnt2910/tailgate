...
{
    MemoryStream stream("POST /tailgate HTTP/1.1\r\nHost: relay.example\r\nConnection: Upgrade\r\n"
                        "Upgrade: tailgate\r\n\r\n");
...
TEST(Given_RelayFrameCoalescedWithHttpUpgrade, When_ClientUpgrades_Then_FrameIsPreserved)
{
    ...
    decoder.Feed(tailgate::relay::RequestHttpUpgrade(stream, "relay.example:443"));
    const std::optional<tailgate::relay::Frame> decoded = decoder.Next();
...
{
    MemoryStream stream("POST /other HTTP/1.1\r\nHost: relay.example\r\nUpgrade: tailgate\r\n\r\n");
...
TEST(Given_OnlineDiscoPeer, When_BuildingDiscoProbes_Then_PingTargetsThatPeer)
{
    ...
    EXPECT_EQ(1U, probes.size());
    const auto message = receiver.Parse(probes.front().Payload);
    EXPECT_TRUE(message.has_value());

    EXPECT_EQ(peerNodePublic, probes.front().Peer);
    EXPECT_TRUE(probes.front().Disco);
    EXPECT_TRUE(message->Type == tailgate::protocol::Disco::MessageType::Ping);
    EXPECT_EQ(sender.PublicKey(), message->Sender);
}
