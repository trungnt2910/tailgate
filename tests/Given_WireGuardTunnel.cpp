...
TEST(Given_EstablishedWireGuardSession, When_SendingKeepalive_Then_EmptyPayloadRoundTrips)
{
    ...
    const auto completed = first.ProcessPacket(firstPeer, accepted->Reply);
    ASSERT_TRUE(completed.has_value());
    EXPECT_TRUE(second.ProcessPacket(secondPeer, completed->Reply).has_value());
...
TEST(Given_EstablishedWireGuardSession, When_Rekeyed_Then_BidirectionalDataUsesNewSession)
{
    ...
    EXPECT_TRUE(initialResponse.has_value());
    const auto initialCompletion = initiator.ProcessPacket(initiatorPeer, initialResponse->Reply);
    EXPECT_TRUE(initialCompletion.has_value());
    EXPECT_TRUE(responder.ProcessPacket(responderPeer, initialCompletion->Reply).has_value());
    ...
    EXPECT_TRUE(rekeyResponse.has_value());
    ...
    EXPECT_TRUE(rekeyCompletion.has_value());
    EXPECT_TRUE(responder.ProcessPacket(responderPeer, rekeyCompletion->Reply).has_value());
    ...
    EXPECT_TRUE(decryptedRequest.has_value());
    EXPECT_TRUE(decryptedRequest->Plaintext == request);
    ...
    EXPECT_TRUE(decryptedResponse.has_value());
    EXPECT_TRUE(decryptedResponse->Plaintext == response);
}
...
TEST(Given_DuplicateHandshakeInitiation, When_OneResponseIsConfirmed_Then_SessionRemainsUsable)
{
    ...
    EXPECT_TRUE(responder.ProcessPacket(responderPeer, completion->Reply).has_value());
    const std::vector<std::uint8_t> request{0x45, 0x00, 0x00, 0x04};
    const auto encryptedRequest = initiator.Encrypt(initiatorPeer, request);
    const auto decryptedRequest = responder.ProcessPacket(responderPeer, encryptedRequest);
