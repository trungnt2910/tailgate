...
TEST(Given_AcmeClient, When_DnsAuthorizationSucceeds_Then_ItReturnsIssuedCertificate)
{
    ...
    http.Responses = {
        Response(
            200,
            R"({"newNonce":"https://ca.example.com/nonce","newAccount":"https://ca.example.com/account","newOrder":"https://ca.example.com/order"})"),
        Response(204, ""),
        Response(201, R"({"status":"valid"})", {{"location", "https://ca.example.com/account/1"}}),
        Response(
            201,
            R"({"authorizations":["https://ca.example.com/auth/1"],"finalize":"https://ca.example.com/finalize/1"})",
            {{"location", "https://ca.example.com/order/1"}}),
        Response(
            200,
            R"({"challenges":[{"type":"dns-01","token":"token","url":"https://ca.example.com/challenge/1"}]})"),
    ...
    Publisher publisher;
    Waiter waiter;
    AcmeClient client(http, crypto, publisher, waiter, "https://ca.example.com/directory");
