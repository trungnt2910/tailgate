...
AcceptFailureAction ClassifyAcceptFailure(int error, bool stopping) noexcept
{
    return !stopping && error == EINTR ? AcceptFailureAction::Retry : AcceptFailureAction::Stop;
}
...
{
    for (const tailgate::net::packet::Ipv4Prefix& route : current.Routes())
    {
        if (std::find(next.Routes().begin(), next.Routes().end(), route) == next.Routes().end())
        {
            RemoveRoute(interfaceName, route);
        }
    }
    for (const tailgate::net::packet::Ipv4Prefix& route : next.Routes())
    {
        if (std::find(current.Routes().begin(), current.Routes().end(), route) ==
            current.Routes().end())
        {
...
tailgate::net::Endpoint ResolveIpv4UdpEndpoint(const std::string& host, int port)
{
    addrinfo hints{};
    ...
    return ToEndpoint(*reinterpret_cast<sockaddr_in*>(entries->ai_addr));
}
