...
public:
    ...
    void UpdatePeers(const std::vector<control::PeerConfig>& configs, std::string exitNode)
    {
        ...
        Routes.clear();
        for (const control::PeerConfig& config : configs)
        {
            Bytes32 publicKey{};
            try
            {
    ...
    {
        const std::optional<std::uint32_t> destination = network::Ipv4Destination(packet);
        if (!destination)
        {
            return nullptr;
        }
        std::vector<control::PeerConfig> configs;
        ...
        const std::optional<std::size_t> exit =
            ExitNode.empty() ? std::nullopt : control::FindExitNode(configs, ExitNode, true);
        const std::optional<std::size_t> route = control::FindRoute(configs, *destination, exit);
...
void WireGuardRouter::UpdatePeers(const std::vector<control::PeerConfig>& peers,
                                  std::string exitNode)
{
...
WireGuardRouter::Send(const std::vector<std::uint8_t>& plaintext)
{
    ...
    if (peer == nullptr)
    {
        const std::optional<std::uint32_t> destination = network::Ipv4Destination(plaintext);
        ...
        if (m_impl->ReportedUnroutableDestinations.emplace(version, destination.value_or(0)).second)
        {
            Log(LogLevel::Warning,
                ...
                std::format("dropping plaintext packet without a peer route destination={} "
                            "version={} bytes={}",
                            destination ? network::FormatIpv4(*destination) : "non-ipv4",
                            version,
                            plaintext.size()));
        }
        return {};
