...
std::optional<std::size_t> PayloadOffset(const std::vector<std::uint8_t>& packet)
{
    if (packet.size() >= Ipv4MinimumHeaderSize && (packet[0] >> 4U) == 4 &&
        network::Ipv4Protocol(packet) == TsmpIpProtocol)
    {
...
BuildTsmpPing(std::uint32_t source, std::uint32_t destination, const TsmpToken& token)
{
    ...
    return network::BuildIpv4Packet(source, destination, TsmpIpProtocol, payload);
}
                                                       ...
                                                       std::uint16_t peerApiPort)
{
    ...
    if ((packet[0] >> 4U) == 4)
    {
        const std::optional<std::uint32_t> source = network::Ipv4Source(packet);
        const std::optional<std::uint32_t> destination = network::Ipv4Destination(packet);
        if (!source || !destination)
        {
            return std::nullopt;
        }
        return network::BuildIpv4Packet(*destination, *source, TsmpIpProtocol, payload);
    }
