...
protocol::Bytes32 DecodeBytes32(const nlohmann::json& value, const char* name, const char* error)
{
    const std::vector<std::uint8_t> bytes = protocol::HexToBytes(value.at(name).get<std::string>());
...
std::string EncodeBytes32(const protocol::Bytes32& value)
{
    return protocol::BytesToHex(value.data(), value.size());
}

protocol::Bytes32 CreateProof(const char* context,
...
{
    const protocol::Bytes32 shared = protocol::X25519Shared(privateKey, publicKey);
    ...
    return protocol::HmacBlake2s256(shared.data(), shared.size(), message.data(), message.size());
}
...
protocol::Bytes32 CreateClientProof(const protocol::Bytes32& clientPrivateKey,
...
protocol::Bytes32 CreateServerProof(const protocol::Bytes32& relayPrivateKey,
...
bool ProofMatches(const protocol::Bytes32& expected, const protocol::Bytes32& actual)
{
...
PeerPacket DecodePeerPacket(const std::vector<std::uint8_t>& payload)
{
    ...
    if (payload.size() <= protocol::Bytes32{}.size() + FlagsSize)
    {
...
{
    if (payload.size() != sizeof(std::uint64_t) + protocol::Bytes32{}.size())
    {
...
std::vector<std::uint8_t> EncodeNetworkConfig(const control::NetworkConfig& config)
{
    nlohmann::json peers = nlohmann::json::array();
    for (const control::PeerConfig& peer : config.Peers)
    {
        nlohmann::json prefixes = nlohmann::json::array();
        for (const network::Ipv4Prefix& prefix : peer.AllowedPrefixes)
        {
    ...
    for (const control::UserProfile& profile : config.UserProfiles)
    {
...
control::NetworkConfig DecodeNetworkConfig(const std::vector<std::uint8_t>& payload)
{
    const nlohmann::json value = ParseJson(payload);
    ...
    for (const nlohmann::json& source : value.value("UserProfiles", nlohmann::json::array()))
    {
        config.UserProfiles.push_back(
            control::UserProfile{.Id = source.value("Id", std::uint64_t{0}),
...
std::vector<PeerPacket> BuildDiscoProbes(const protocol::Disco& disco,
                                         const std::vector<control::PeerConfig>& peers)
{
    ...
    for (const control::PeerConfig& peer : peers)
    {
        ...
        if (nodeBytes.size() != protocol::Bytes32{}.size() ||
        ...
        PeerPacket probe;
        probe.Peer = nodeKey;
        probe.Payload = disco.BuildPing(discoKey, disco.NewTransactionId());
        probe.Disco = true;
