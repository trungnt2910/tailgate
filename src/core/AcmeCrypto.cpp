...
public:
    ...
    {
        const auto raw = protocol::Base64Decode(encoded);
...
MbedTlsCrypto::MbedTlsCrypto() : m_context(std::make_unique<tailgate::detail::PsaCryptoContext>())
{
}
...
std::string MbedTlsCrypto::GeneratePrivateKey()
{
    ...
    return protocol::Base64Encode({raw.begin(), raw.begin() + static_cast<std::ptrdiff_t>(size)});
}
