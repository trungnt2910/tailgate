...
MbedTlsCertificate::MbedTlsCertificate()
    : m_context(std::make_unique<tailgate::crypto::detail::PsaCryptoContext>())
{
}

MbedTlsCertificate::~MbedTlsCertificate() = default;

std::string MbedTlsCertificate::GeneratePrivateKey()
{
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    psa_set_key_type(&attributes, PSA_KEY_TYPE_ECC_KEY_PAIR(PSA_ECC_FAMILY_SECP_R1));
    psa_set_key_bits(&attributes, 256);
...
std::string MbedTlsCertificate::Jwk(const std::string& privateKey)
{
    Key key(privateKey);
...
std::vector<std::uint8_t> MbedTlsCertificate::Sign(const std::string& privateKey,
...
std::string MbedTlsCertificate::Thumbprint(const std::string& privateKey)
{
...
Bytes32 MbedTlsCertificate::Sha256(std::string_view input)
{
    Bytes32 digest{};
    std::size_t digestSize = 0;
    Check(psa_hash_compute(PSA_ALG_SHA_256,
...
MbedTlsCertificate::CreateCertificateRequest(const std::string& privateKey,
...
std::string MbedTlsCertificate::ToPem(const std::string& privateKey)
{
    Key key(privateKey);
    mbedtls_pk_context pk{};
    mbedtls_pk_init(&pk);
...
bool MbedTlsCertificate::CertificateValidFor(const std::string& certificatePem,
